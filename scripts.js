...
particlesJS('particles-js', {
    ...
    const textArray = ['MERN Stack Developer', 'Full Stack Developer', 'Freelancer'];
    let textIndex = 0;
    let charIndex = 0;
    let currentText = '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateProjectItem(item) {
        ...
        gsap.fromTo(buttons,
            { opacity: 0, y: 50 },
            { opacity: 1, y: 0, duration: 1, stagger: 0.2, delay: 0.8 }
        );
    }

    function resetProjectItem(item) {
        const projectInfo = item.querySelector('.project-info');
        const buttons = item.querySelectorAll('.btn');

        gsap.set(projectInfo, { opacity: 0, y: 50 });
